...
import {collection, query, orderBy, where, getDocs, type CollectionReference} from 'firebase/firestore';
...
export const GET = async (req: NextRequest) => {
	...
	if (name !== '') {
		...
		if (nameSort !== '') {
			switch (nameSort) {
				case 'name_asc':
					newProducts = newProducts.sort((a, b) => a.name.toLowerCase().localeCompare(b.name.toLowerCase()));
					break;

				case 'name_desc':
					newProducts = newProducts.sort((a, b) => b.name.toLowerCase().localeCompare(a.name.toLowerCase()));
					break;
